...
def main():
    ...
    parser.add_argument('--log-interval',
                        ...
                        default=10,
                        metavar='N',
                        help='batches number before logging training status')
    parser.add_argument('--save-model', action='store_true', default=True, help='For Saving the current Model')
    parser.add_argument('--mode', default='train', help='train or test model')
    ...
    test_loader = DataLoader(dataset=test_set, batch_size=args.test_batch_size, shuffle=True, **kwargs)
    ...
    if (args.mode == 'test'):
        load_path = '../data/mnist_cnn.pt'
        if not load_path:
            raise FileNotFoundError('dont have saved model')
        model.load_state_dict(torch.load(load_path))
        test(args, model, device, test_loader)

    if (args.save_model):
        torch.save(model.state_dict(), "../data/MNIST/mnist_cnn.pt")
